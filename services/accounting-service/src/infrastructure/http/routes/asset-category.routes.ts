...
import { GetAssetCategoryHandler, ListAssetCategoriesHandler } from '@/application/queries';
 ...
 */
assetCategoryRoutes.get('/', zValidator('query', listAssetCategoriesQuerySchema), async (c) => {
  const db = c.get('db');
 ...
 */
assetCategoryRoutes.post('/', zValidator('json', createAssetCategorySchema), async (c) => {
  const db = c.get('db');
 ...
 */
assetCategoryRoutes.put('/:id', zValidator('json', updateAssetCategorySchema), async (c) => {
  const db = c.get('db');
  const id = c.req.param('id');
  const body = c.req.valid('json');
