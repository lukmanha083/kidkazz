...
import { categoryStatusOptions, getCategoryColumns } from '@/components/ui/data-table/columns';
import {
  Drawer,
  DrawerClose,
  DrawerContent,
...
function CategoryPage() {
  ...
  // Sort categories hierarchically: parents first, then their children directly underneath
  const categories = useMemo(() => {
    const parents = rawCategories.filter((cat) => !cat.parentId);
    const result: Category[] = [];

    for (const parent of parents) {
      result.push(parent);
      // Find and add all children of this parent
      const children = rawCategories.filter((cat) => cat.parentId === parent.id);
      result.push(...children);
    }

    // Add any orphaned categories (children without a parent in the list)
    const addedIds = new Set(result.map((cat) => cat.id));
      ...
      <Drawer open={formDrawerOpen} onOpenChange={setFormDrawerOpen}>
        <DrawerContent side="left">
          <DrawerHeader className="relative">
            ...
            <DrawerTitle>{formMode === 'add' ? 'Add New Category' : 'Edit Category'}</DrawerTitle>
            <DrawerDescription>
              {formMode === 'add' ? 'Create a new product category' : 'Update category information'}
            </DrawerDescription>
          </DrawerHeader>
              ...
              {(field) => (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Only top-level categories can be selected as parents. We support 2 levels:
