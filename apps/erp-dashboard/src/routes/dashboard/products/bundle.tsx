...
function ProductBundlePage() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: form.getFieldValue and getProductStock are stable helpers
  ...
  const generateBundleSKU = (bundleName: string) => {
    ...
    const existingBundles = bundles.filter((b) => b.bundleSKU.startsWith(`${nameCode}-`));
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: calculateOriginalPrice is a stable helper function
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage product bundles and packages</p>
          </div>
        </div>
        <Card>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>View and manage your product bundles</CardDescription>
        </CardHeader>
        <CardContent>
          <DataTable
            columns={columns}
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {(selectedBundle as any).warehouseId
                        ? warehouses.find((w) => w.id === (selectedBundle as any).warehouseId)
              ...
              <div className="space-y-4 border-t pt-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="font-medium">
                      {formatRupiah(calculateOriginalPrice(selectedBundleItems))}
                    </p>
                  </div>
                  <div>
              ...
              <div className="space-y-4 border-t pt-4">
                ...
                <Tabs defaultValue="images" className="w-full">
                  ...
                  <TabsContent value="images" className="mt-4">
                    <ImageGallery productId={selectedBundle.id} readOnly={true} />
                  </TabsContent>
                  <TabsContent value="videos" className="mt-4">
                    <VideoGallery productId={selectedBundle.id} readOnly={true} />
                  </TabsContent>
                </Tabs>
              </div>
            </div>
      ...
      <Drawer open={formDrawerOpen} onOpenChange={setFormDrawerOpen}>
        <DrawerContent side="left">
          <DrawerHeader>
            <DrawerTitle>{formMode === 'add' ? 'Create New Bundle' : 'Edit Bundle'}</DrawerTitle>
            <DrawerDescription>
              {formMode === 'add'
            ...
            <form.Field name="warehouseId">
              {(field) => (
                <div className="space-y-2 border rounded-lg p-3 bg-blue-50/50 dark:bg-blue-950/20">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Select the warehouse where this bundle will be assembled. All component products
                    must be available in this warehouse.
                  </p>
                </div>
              )}
            </form.Field>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => handleRemoveProductFromBundle(item.productId)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
              ...
              <div className="p-4 bg-muted/50 border border-border rounded-lg space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-destructive">
                    -
                    {formatRupiah(
                      (calculateOriginalPrice(selectedProducts) * (discountPercentage || 0)) / 100
                    )}
                  </span>
                </div>
                <Separator />
              ...
              <div className="space-y-2">
                ...
                <div className="p-4 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900 rounded-lg space-y-3">
                  ...
                  <p className="text-xs text-blue-700 dark:text-blue-300 mt-2">
                    💡 Bundle stock is calculated in real-time from component availability. No stock
                    is pre-allocated.
                  </p>
                </div>
              </div>
              ...
              <Tabs defaultValue="images" className="w-full mt-3">
                ...
                <TabsContent value="images" className="mt-4">
                  <div className="space-y-3">
                    <p className="text-xs text-muted-foreground">
                      Upload and manage bundle images. Supports multiple images with automatic
                      optimization.
                    </p>
                    {selectedBundle?.id ? (
                      <ImageGallery productId={selectedBundle.id} readOnly={false} />
                    ) : (
                ...
                <TabsContent value="videos" className="mt-4">
                  <div className="space-y-3">
                    <p className="text-xs text-muted-foreground">
                      Upload and manage bundle videos. Choose between R2 storage or Cloudflare
                      Stream.
                    </p>
                    {selectedBundle?.id ? (
                      <VideoGallery productId={selectedBundle.id} readOnly={false} />
                    ) : (
                      <p className="text-sm text-muted-foreground text-center py-8">
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete "{bundleToDelete?.bundleName}"? This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
