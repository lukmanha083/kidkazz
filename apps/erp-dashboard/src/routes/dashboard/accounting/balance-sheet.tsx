import { type BalanceSheet, accountingApi } from '@/lib/api';
      ...
      {!report ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="text-center mb-6 pb-4 border-b border-gray-200">
            ...
            <p className="text-gray-600 mt-1">As of {new Date(report.asOf).toLocaleDateString()}</p>
          </div>

          {/* Balance Check Alert */}
          <div
            className={`flex items-center gap-2 p-4 rounded-lg mb-6 ${
              report.isBalanced ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'
            }`}
          >
            {report.isBalanced ? (
              <>
          ...
          <div className="mt-6 pt-6 border-t-2 border-gray-300">
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="flex items-center justify-center gap-4 text-lg">
                <span className="font-medium">Assets: ${report.assets.total.toLocaleString()}</span>
                <span className="text-gray-400">=</span>
                <span className="font-medium">
                  Liabilities: ${report.liabilities.total.toLocaleString()}
                </span>
                <span className="text-gray-400">+</span>
                <span className="font-medium">Equity: ${report.equity.total.toLocaleString()}</span>
              </div>
              {!report.isBalanced && (
                <div className="text-center text-red-600 mt-2 text-sm">
                  Difference: $
                  {Math.abs(
                    report.assets.total - report.totalLiabilitiesAndEquity
                  ).toLocaleString()}
                </div>
              )}
            </div>
