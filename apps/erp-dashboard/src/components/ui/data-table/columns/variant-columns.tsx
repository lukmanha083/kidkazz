...
export function getVariantColumns(options: VariantColumnOptions = {}): ColumnDef<ProductVariant>[] {
  ...
  return [
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title="Variant Name" />,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge className={`${getVariantTypeBadge(type)} hidden lg:table-cell`}>{type}</Badge>
        );
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge
            ...
            className={`${
              isActive ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-500' : ''
