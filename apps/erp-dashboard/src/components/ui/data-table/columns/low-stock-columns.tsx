...
export function getLowStockColumns(): ColumnDef<LowStockItem>[] {
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => <span className="text-muted-foreground">{row.getValue('warehouse')}</span>,
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className={isCritical ? 'text-destructive font-medium' : ''}>{currentStock}</span>
        );
      },
    },
    {
      ...
      cell: ({ row }) => <span className="text-muted-foreground">{row.getValue('minStock')}</span>,
    },
    {
      accessorKey: 'status',
