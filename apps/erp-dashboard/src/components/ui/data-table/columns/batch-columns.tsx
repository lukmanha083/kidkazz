 ...
 */
export function getBatchColumns(options: BatchColumnOptions = {}): ColumnDef<InventoryBatch>[] {
  ...
  return [
    {
      ...
      cell: ({ row }) => <span className="font-medium">{row.getValue('batchNumber')}</span>,
    },
    {
      accessorKey: 'lotNumber',
      header: ({ column }) => <DataTableColumnHeader column={column} title="Lot #" />,
      cell: ({ row }) => (
        <span className="text-sm text-muted-foreground">{row.getValue('lotNumber') || '-'}</span>
      ),
    },
    {
      id: 'expiration',
      ...
      sortingFn: (rowA, rowB) => {
        const daysA = calculateDaysUntilExpiration(rowA.original.expirationDate);
        const daysB = calculateDaysUntilExpiration(rowB.original.expirationDate);
        ...
        return (
          <div className="space-y-1">
            ...
            {batch.quantityReserved > 0 && (
              <div className="text-sm text-muted-foreground">{batch.quantityReserved} reserved</div>
            )}
          </div>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm text-muted-foreground">{row.getValue('supplier') || '-'}</span>
      ),
    },
    {
      id: 'actions',
